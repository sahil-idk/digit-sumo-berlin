...
class ModernTrafficGUI:
    ...
    def run_ml_prediction(self):
        ...
        try:
            ...
            # Prepare input data based on the source
            if data_source == "Current Simulation":
                input_sequence = self.prepare_simulation_data(seq_length)
            else:  # Historical data
                csv_path = self.csv_path_var.get()
                input_sequence = self.prepare_csv_data(csv_path, seq_length)
            
            if input_sequence is None:
                self.prediction_status_var.set("Error: Failed to prepare input data")
                return
            
            # Update status
            self.prediction_status_var.set("Running prediction model...")
            self.root.update()
            ...
            try:
                ...
                if predictions is not None:
                    ...
                    self.display_predictions(input_sequence, predictions)
                    self.prediction_status_var.set("Prediction completed successfully")
                else:
                    self.prediction_status_var.set("Error: Prediction failed")
            except Exception as e:
    ...
    def prepare_simulation_data(self, seq_length):
        ...
        try:
            # Get current vehicle counts from the RSUs
            vehicle_counts = []
            
            # If we have RSU data, use it
            if self.rsus:
                # Use the sum of vehicle counts from all RSUs
                for _ in range(seq_length):
                    count = sum(len(rsu.vehicles_in_range) for rsu in self.rsus)
                    vehicle_counts.append(count)
            else:
                # Otherwise use the total vehicle count in the simulation
                for _ in range(seq_length):
                    vehicles = traci.vehicle.getIDList()
                    vehicle_counts.append(len(vehicles))
                    
                    # Step the simulation to get the next count
                    if _ < seq_length - 1:  # Don't step after the last count
                        traci.simulationStep()
            
            # Format for LSTM: (1, sequence_length, 1)
            input_sequence = np.array(vehicle_counts).reshape(1, seq_length, 1)
    ...
    def prepare_csv_data(self, csv_path, seq_length):
        """Prepare input sequence from CSV data"""
        try:
            # Load the CSV file
            df = pd.read_csv(csv_path)
            
            # Check for the right columns - assume we need Flow data
            flow_columns = [col for col in df.columns if 'Flow' in col]
            
            if not flow_columns:
                messagebox.showerror("Error", "No 'Flow' columns found in the CSV file")
                return None
            ...
            if len(df) < seq_length:
                messagebox.showerror("Error", f"CSV file has fewer than {seq_length} rows")
                return None
            
            # Get the last seq_length records
            last_values = df[flow_column].values[-seq_length:]
    ...
    def display_predictions(self, input_sequence, predictions):
        """Display the prediction results on the chart"""
        # Reshape data for plotting
        input_data = input_sequence.reshape(-1)
        pred_data = predictions.reshape(-1)
        
        # Create time steps
        time_steps = list(range(len(input_data) + len(pred_data)))
        
        # Create data points
        historical_data = np.concatenate([input_data, np.zeros_like(pred_data) * np.nan])
        future_data = np.concatenate([np.zeros_like(input_data) * np.nan, pred_data])
        
        # Clear previous plot
        self.pred_ax.clear()
        
        # Plot historical data
        self.pred_ax.plot(time_steps, historical_data, 'o-', color=COLORS["primary"], 
                         label='Historical Data')
        
        # Plot predicted data
        self.pred_ax.plot(time_steps, future_data, 'o--', color=COLORS["danger"], 
                         label='Predicted Data')
        
        # Mark the current time point
        self.pred_ax.axvline(x=len(input_data)-1, color='black', linestyle='--', alpha=0.7)
        self.pred_ax.annotate('Now', xy=(len(input_data)-1, 0), xytext=(len(input_data)-1, -5),
                            ha='center', va='top')
        ...
        self.display_prediction_table(input_data, pred_data)

    def display_prediction_table(self, input_data, pred_data):
        ...
        for widget in self.prediction_results.winfo_children():
            if widget != self.pred_canvas.get_tk_widget():
                widget.destroy()
        ...
        headers = ["Time Step", "Historical Value", "Predicted Value"]
        
        # Calculate the future time steps
        current_time = traci.simulation.getTime()
        time_step = 5  # assuming 5-minute intervals
        ...
        for i in range(len(pred_data)):
            # Time step (future)
            time_label = f"Now + {(i+1)*time_step}min"
            
            # Add row
            row_color = "white" if i % 2 == 0 else "#f8f8f8"
            
            tk.Label(table, text=time_label, font=("Segoe UI", 9), 
                    bg=row_color, padx=10, pady=5, borderwidth=1, relief="solid").grid(row=i+1, column=0, sticky="nsew")
            
            tk.Label(table, text="N/A", font=("Segoe UI", 9), 
                    bg=row_color, padx=10, pady=5, borderwidth=1, relief="solid").grid(row=i+1, column=1, sticky="nsew")
            
            tk.Label(table, text=f"{pred_data[i]:.2f}", font=("Segoe UI", 9), 
                    bg=row_color, fg=COLORS["danger"], padx=10, pady=5, borderwidth=1, relief="solid").grid(row=i+1, column=2, sticky="nsew")
        ...
        StylishButton(export_frame, text="Export Predictions", color="primary",
                     command=lambda: self.export_predictions(input_data, pred_data)).pack(side="right")

    def export_predictions(self, input_data, pred_data):
        ...
        try:
            ...
            # Create time steps for historical data
            historical_times = [current_time - pd.Timedelta(minutes=(len(input_data)-i)*5) 
                               for i in range(len(input_data))]
            
            # Create time steps for prediction data
            prediction_times = [current_time + pd.Timedelta(minutes=(i+1)*5) 
                               for i in range(len(pred_data))]
            
            # Combine the data
            all_times = historical_times + prediction_times
            all_values = list(input_data) + [None] * len(pred_data)
            all_predictions = [None] * len(input_data) + list(pred_data)
            
            # Create a DataFrame
            df = pd.DataFrame({
                "Timestamp": all_times,
                "Historical Value": all_values,
