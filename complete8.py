...
class ScenarioManager:
    ...
    def load_vehicle_spawn_data(self):
        """
        Load vehicle spawn schedule from FinalV2C_Dataset.csv.
        This will simulate how many vehicles should be inserted at different timestamps.
        """
        try:
            df = pd.read_csv("FinalV2C_Dataset.csv")

            if 'timestamp' not in df.columns or 'vehicle_type' not in df.columns:
                print("CSV must contain 'timestamp' and 'vehicle_type'")
                return []

            # Group vehicle count by timestamp for spawning schedule
            spawn_schedule = (
                df.groupby('timestamp')
                .agg(vehicle_count=('vehicle_type', 'count'))
                .reset_index()
                .sort_values('timestamp')
            )

            self.vehicle_spawn_df = df
            self.vehicle_spawn_schedule = spawn_schedule
            return True

        except Exception as e:
            print(f"Error loading vehicle spawn data: {e}")
            return False
