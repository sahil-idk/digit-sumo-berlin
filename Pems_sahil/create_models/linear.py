...
def prepare_data(data, input_window=10, output_window=5):
    X, Y = [], []
...
def save_flow_and_timestamp(raw_data, folder_path='../models/linear/'):
...
# Assuming you have already loaded the data into a pandas DataFrame named 'data'
# Split data into train, validation, and test sets
train_size = int(0.50 * len(data))
...
# Save the raw training data (50% of the original data) with timestep information
# save_raw_training_data(train_data)
...
model = LinearRegressionModel(input_dim * input_window, output_dim, output_window)
...
model_path = '../models/linear/linear_model.pth'
torch.save(best_model, model_path)
torch.save(scaler, '../models/linear/scaler.pth')
...
def store_baseline_predictions(model, data_loader, scaler):
    ...
    # Save to CSV
    df.to_csv('../models/linear/baseline_predictions_linear.csv', index=False)
    print("Baseline predictions saved to 'baseline_predictions_linear.csv'")
