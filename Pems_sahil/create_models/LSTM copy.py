...
# Use only the first month of data to train
first_month_data = data[data.index < data.index[0] + pd.DateOffset(months=1)]

train_size = int(0.5 * len(first_month_data))
val_size = int(0.25 * len(first_month_data))
test_size = len(first_month_data) - train_size - val_size

train_data, val_data, test_data = np.split(first_month_data, [train_size, train_size + val_size])
...
# Save the model
torch.save(model.state_dict(), '../models/lstm2/lstm2_model.pth')
torch.save(scaler, '../models/lstm2/lstm2_scaler.pth')
...
def store_baseline_predictions(model, data_loader, scaler):
    ...
    # Save to CSV
    df.to_csv('../models/lstm2/baseline_predictions_lstm2.csv', index=False)
    print("Baseline predictions saved to 'baseline_predictions_lstm2.csv'")
