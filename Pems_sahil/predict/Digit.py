...
def predict(scaled_input_sequence):
    try:
        ...
        # Extract the fifth timestep prediction for each node
        fifth_timestep_predictions = [pred[4] for pred in predictions_inverse]
        with open('predicted_results.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(fifth_timestep_predictions)
...
if predicted_values is not None:
    ...
    # Store the predicted results in a csv
