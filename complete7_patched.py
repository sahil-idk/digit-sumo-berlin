"""
Enhanced Traffic Simulation GUI with integrated ML prediction
"""
...
def generate_synthetic_vehicle_counts(csv_path, total_timesteps=300):
    """
    Uses trends in CSV to create synthetic vehicle counts over time.
    Expects the CSV to have a 'vehicle_count' or similar column.
    """
    try:
        df = pd.read_csv(csv_path)
        if 'vehicle_count' not in df.columns:
            raise ValueError("CSV must have a 'vehicle_count' column")

        mean_count = df['vehicle_count'].mean()
        std_count = df['vehicle_count'].std()

        synthetic_counts = np.clip(
            np.random.normal(loc=mean_count, scale=std_count, size=total_timesteps).astype(int),
            0, None
        )

        return list(enumerate(synthetic_counts))  # (timestep, count)

    except Exception as e:
        print(f"[Synthetic Generator Error] {e}")
        return None

def inject_synthetic_vehicles(vehicle_counts, edges, vehicle_params):
    for timestep, count in vehicle_counts:
        for i in range(count):
            veh_id = f"veh_{timestep}_{i}"
            ...
            traci.vehicle.setColor(veh_id, traci.gui.Color(*vehicle_params["color"]))
            traci.vehicle.setMaxSpeed(veh_id, vehicle_params["speed"])

def run_vehicle_generation(csv_path, edges, vehicle_params, fallback_callback):
    synthetic_data = generate_synthetic_vehicle_counts(csv_path)
    if synthetic_data is not None:
        print("[INFO] Using synthetic generator for vehicle injection.")
        inject_synthetic_vehicles(synthetic_data, edges, vehicle_params)
    else:
        print("[WARNING] Falling back to default vehicle generation.")
        fallback_callback()
